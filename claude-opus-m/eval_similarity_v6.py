#!/usr/bin/env python3
"""
人脸特征相似度评估系统 - 第六版：彻底修复版本
关键修正：
1. 每个GPU处理行范围[row_start, row_end)
2. 对于每一行i，计算与**整个矩阵**中所有j > i的相似度（不仅仅是当前GPU之后的行）
3. 这样才能捕获所有跨GPU边界的正样本对
"""
import pickle
import numpy as np
import torch
...
def process_row_range(rank, features, query_ids, row_start, row_end, chunk_size, device, return_dict):
    """
    处理指定行范围的相似度计算
    关键：对于行范围[row_start, row_end)中的每一行i，
    计算与整个矩阵中所有j > i的列的相似度
    """
    try:
        N = len(features)
        ...
        for i in range(row_start, row_end, chunk_size):
            ...
            # 对于当前块的每一行，计算与所有j > 该行最大索引 的列
            # 注意：j要从i_end开始（而不是i+1），这样确保j > i对于块内所有行都成立
            # 但这会漏掉块内的上三角部分，所以需要分两部分处理

            # 第1部分：处理块内的上三角
            # 对于块内的行i_local，只计算与块内j_local > i_local的相似度
            for i_local in range(i_end - i):
                global_i = i + i_local
                feat_i_single = feat_tensor[global_i:global_i+1]  # (1, 512)
                id_i_single = query_ids_tensor[global_i:global_i+1]  # (1,)
