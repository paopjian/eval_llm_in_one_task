#!/usr/bin/env python3
"""
人脸特征相似度评估系统 - 第五版：完全修复版本
核心修复：
1. 每个GPU处理自己的行范围 [row_start, row_end)
2. 对于每一行i，计算与所有j > i的列的相似度
3. 正确处理上三角矩阵，确保不重复不遗漏
"""
import pickle
import numpy as np
import torch
...
def process_row_range(rank, features, query_ids, row_start, row_end, chunk_size, device, return_dict):
    """
    处理指定行范围的相似度计算
    对于行范围[row_start, row_end)中的每一行i，计算与所有j > i的相似度
    """
    try:
        N = len(features)
        ...
        for i in range(row_start, row_end, chunk_size):
            ...
            # 对于每一行，计算与j > i的所有列
            # j的起始位置是i+1，但由于我们是分块的，需要从当前块的最大索引+1开始
            j_start = i_end  # 确保j > i
