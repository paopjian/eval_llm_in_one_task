...
# -*- coding: utf-8 -*-
"""
A/B 实验：glm-线程版 vs 基准 cluster_utils get_sim_matrix_large_scale_v5。

    baseline (cores/baseline.py -> cluster_utils v5): 动态块池(工作量降序, 首1%后0.5%)
        + 10240 分块 + 20M bins 直方图(后聚合到 200K) + masked_fill(NaN)+histc
    glm-线程版 (experiments/glm_threaded.py): 动态 tile 队列(宽优先) + 16384 分块
        + 200K bins 直方图 + 每 tile 一次 histc

用法:
    python benchmark/experiments/test_baseline_vs_glm_threads.py --data test_data_10min.pkl   # 200K
    python benchmark/experiments/test_baseline_vs_glm_threads.py --data test_data_200w.pkl   # 2M
输出: logs/unified_eval/baseline_vs_glm_threads/summary.json
"""
import argparse
import json
import os
...
OUT = os.path.join('logs', 'unified_eval', 'baseline_vs_glm_threads')
...
def ab(data_path, gpus):
    ...
    results = []
    rb, pos_b, neg_b = run_core(base_core, 'baseline(cluster_utils v5)', feats, ids, gpus,
                                os.path.join(OUT, 'work', 'baseline'))
    results.append(rb)
    rt, pos_t, neg_t = run_core(glm_thr, 'glm-线程版(glm_threaded.py)', feats, ids,
                                gpus, os.path.join(OUT, 'work', 'glm_threads'))
    results.append(rt)

    same = bool((pos_b == pos_t).all() and (neg_b == neg_t).all())
    diff = int(abs(pos_b.astype('int64') - pos_t.astype('int64')).sum()) + \
           int(abs(neg_b.astype('int64') - neg_t.astype('int64')).sum())
    ...
    if rb['status'] == 'success' and rt['status'] == 'success':
        speed = rb['wall_s'] / rt['wall_s']
        print(f'\nglm-线程版相对 baseline: 速度 {"快" if speed > 1 else "慢"} {abs(speed-1)*100:.1f}% '
              f'(baseline wall/线程 wall={speed:.3f})；'
              f'峰值RSS 线程/baseline={rt["peak_rss_gb"]/max(rb["peak_rss_gb"],1e-9):.2f}× '
              f'(baseline {rb["peak_rss_gb"]}GB -> 线程 {rt["peak_rss_gb"]}GB)')
