...
MODEL_NAME = 'codex-sol-fp32'
MODEL_DESC = ('Codex(SOL) 方法：静态行块LPT贪心分组(无动态队列) + '
              'fp16分块matmul + 手工bincount直方图(上三角逐块)')
ORIGIN = ('benchmark/experiments/codex_sol_fp32.py 自动生成自 cores/codex_sol.py (原 codex-sol/evaluate_similarity/_partition_ranges/'
          '_compute_ranges/_score_to_torch_hist；bins 4096 -> 统一 200K)')

BLOCK = 2048            # 实际运行的 block_size
NATIVE_BINS = 4096      # 原默认 bins
PRECISION = 'fp32'      # 实验变体: 关闭 half 转换，同调度纯 fp32
